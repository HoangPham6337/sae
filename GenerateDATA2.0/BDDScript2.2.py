...
import os
import time
from deep_translator import GoogleTranslator  # Importation pour la traduction
...
# Fonction pour traduire un texte en français
...
def transform_game_data(games):
    ...
    for game in games:
        ...
        # Séparation des développeurs et éditeurs
        ...
        description_fr = translate_to_french(description_en)
...
def load_progress():
    if not os.path.exists(PROGRESS_FILE):
        # Création d'un nouveau fichier progress.json si absent
        with open(PROGRESS_FILE, "w", encoding="utf-8") as file:
            json.dump({"offset": 0, "games_retrieved": 0}, file, ensure_ascii=False, indent=4)
        print(f"Fichier {PROGRESS_FILE} créé avec succès.")
...
def main():
    ...
    while games_retrieved < 10000:
        ...
        # Délai pour éviter de trop solliciter l'API
        time.sleep(1)
